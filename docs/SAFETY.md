...
SkillsGoat is a benchmark of public fixtures, so results are published openly
(see [SECURITY.md](../SECURITY.md)). A miss is a detection-rate result, not a
0-day. Publish only reproducible, `goat scan --blind`, version-pinned, dated
numbers; vendors have a standing right of reply, and a genuinely novel bypass
gets a short courtesy heads-up before the mechanism is published.
