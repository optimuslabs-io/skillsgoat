...
<!-- Generated by `goat chain-report`. Do not edit by hand; edit the chain.yaml files and regenerate. -->

37 chains. Every node scans CLEAN alone; only the graph is malicious.
...
## c36-reference-reconstruction — Reference Reconstruction Chain (tier 300)
...
```mermaid
graph LR
  bootstrap -- external-reference-rebuild --> reference
  bootstrap[bootstrap]
  reference[reference]
```

**Why:** Nothing executable ships. The bootstrap node points the agent at an external
...
**Blast radius:** credentials=['~/.aws', '~/.env']; persistence=reconstructed script re-runs on every deploy
...
## c37-time-gated-updater — Time-Gated Cross-Skill Updater (tier 300)
...
```mermaid
graph LR
  schedule -- cache-policy-file --> updater
  updater[updater]
  schedule[schedule]
```

**Why:** Neither node is malicious at review. The updater just applies a shared config
...
**Blast radius:** credentials=['~/.aws', '~/.config']; persistence=re-arms every run once the date passes
